...
Usage:
    ...
    python characterize_datasets.py \
        ...
        --beto_cgec          models/beto-cgec \
        --roberta_base       PlanTL-GOB-ES/roberta-base-bne \
        --roberta_cgec       models/roberta-bne-cgec \
        --xlmr_base          xlm-roberta-base \
        --xlmr_cgec          models/xlm-roberta-cgec \
        --output             results/characterization.csv
"""
...
def compute_perplexity(texts, model_path, batch_size=32, max_length=512):
    ...
    try:
        ...
        ppl = math.exp(total_loss / total_tokens) if total_tokens > 0 else None
        return round(ppl, 4) if ppl else None
...
def main(args):
    ...
    perplexity_models = [
        ("linguistic_distance_beto",     args.beto_base),
        ("domain_distance_beto",         args.beto_base),
        ("linguistic_distance_roberta",  args.roberta_base),
        ("domain_distance_roberta",      args.roberta_base),
        ("linguistic_distance_xlmr",     args.xlmr_base),
        ("domain_distance_xlmr",         args.xlmr_base),
    ]
    ...
    for csv_path in csv_files:
        ...
        # Manual columns — empty for human completion
        ...
        # Prepare source representations
        ...
        # Auto — perplexity (linguistic + domain distance per model family)
